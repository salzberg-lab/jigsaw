...
int error(const std::string& error_text)
{
  ...
  // Saves a lot of heart ache by identifying which file is being
  // read to cause the problem in dtree.cc
  throw std::runtime_error(string("util.cc, error(): ")+error_text);
  //return(0);
}
