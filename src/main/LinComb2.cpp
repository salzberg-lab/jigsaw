...
LinComb2* LinComb2::_instance = NULL;

//static AnnotationScore getProb(const RegionPrediction&, EvScores::Bnd_t, dsu::Strand_t, int); 

CustomScore::LabelScore LinComb2::score(const RegionPrediction& rp, const string& str) const {
  assert(0);
...
void LinComb2::score(vector<RegionPrediction*>& vrp, const string& str) const {
...
EvScores LinComb2::scoreHelp(const vector<RegionPrediction*>& rpv, const string& str, dsu::Strand_t strnd) const {
...
EvScores LinComb2::scoreHelpOpp(const vector<RegionPrediction*>& rpv, const string& str, dsu::Strand_t strnd) const {
  // we will assume (for now) that the 
  // we are scoring the middle rp of a odd number of vectors
  int middle = (rpv.size()-1)/2;
  if( rpv[middle]->noStrndData(strnd) ) return EvScores();
...
double LinComb2::getProb(const AnnotationItem& item, EvScores::Bnd_t cbnd, dsu::Strand_t strnd) const {
...
AnnotationScore 
LinComb2::getRawProb(const RegionPrediction& rp, EvScores::Bnd_t bnd, dsu::Strand_t strnd) const {
...
*/
AnnotationScore 
LinComb2::getProbHelp(const RegionPrediction& rp, EvScores::Bnd_t bnd, dsu::Strand_t strnd, int exonCnt) const {
  assert(exonCnt >= 0);
